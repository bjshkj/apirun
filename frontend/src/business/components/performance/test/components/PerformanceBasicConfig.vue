...
export default {
  ...
  data() {
    return {
      ...
      threadGroupForSelect: [
        ...
        {
          name: '自定义线程组',
          tagName: 'kg.apc.jmeter.threads.UltimateThreadGroup',
          testclass: 'kg.apc.jmeter.threads.UltimateThreadGroup',
          quiclass: 'kg.apc.jmeter.threads.UltimateThreadGroupGui'
        }
      ]
    };
  },
  ...
  methods: {
    ...
    tgTypeChange(row) {
      if (row.tgType === 'kg.apc.jmeter.threads.UltimateThreadGroup') {
        this.threadGroups.filter(tg => tg.threadNumber = '10');
        this.threadGroups.filter(tg => tg.model = 'shock');
        this.threadGroups.filter(tg => tg.duration = 60);
      }else {
        this.threadGroups.filter(tg => tg.model = '');
      }
