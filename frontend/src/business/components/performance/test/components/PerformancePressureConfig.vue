              ...
              <div v-if="threadGroup.tgType !== 'kg.apc.jmeter.threads.UltimateThreadGroup'">
                ...
                <div v-if="threadGroup.threadType === 'DURATION'">
                  ...
                  <div v-if="threadGroup.tgType === 'com.blazemeter.jmeter.threads.concurrency.ConcurrencyThreadGroup'">
                    ...
                    <el-form-item :label="$t('load_test.ramp_up_time_times')" label-width="120px"/>
                  </div>
  ...
  methods: {
    ...
    _calculateTotalChart() {
      ...
      for (let i = 0; i < handler.threadGroups.length; i++) {
        ...
        if (tg.threadNumber < this.minThreadNumber) {
          this.$error('最小并发数应小于并发用户数');
          return;
        }
        ...
        if (tg.model === 'shock') {
          if (duration < tg.shockCount * 2 + 20 * tg.durationMinutes) {
            this.$error('用例总时间必须大于两倍的次数');
            return;
          }
          this.$delete(handler.options.xAxis, 'type');
          this.$delete(handler.options.xAxis, 'data');
          this.$set(seriesData, 'smooth', true);
          seriesData.data.length = 0;
          //第一个震荡点的时间
          let startTime = duration / tg.shockCount / 2;
          for (let j = 1; j <= tg.shockCount; j++) {
            seriesData.data.push([delayTime, 0]);
            seriesData.data.push([delayTime + startTime, tg.threadNumber]);
            delayTime += duration / tg.shockCount;
          }
          seriesData.data.push([duration, 0]);
          handler.options.series.push(seriesData);
        } else if (tg.model === 'waveSurge') {
          let peakDuration = tg.peakDuration;
          let minDuration = (duration - peakDuration * tg.waveCount) / tg.waveCount;
          if (duration < peakDuration * tg.waveCount) {
            this.$error('用例总时间必须大于峰值持续时间乘以次数');
            return;
          }
          ...
          for (let j = 1; j <= tg.waveCount; j++) {
            seriesData.data.push([delayTime, tg.minThreadNumber]);
            seriesData.data.push([delayTime + minDuration, tg.minThreadNumber]);
            delayTime += minDuration;
            seriesData.data.push([delayTime, tg.threadNumber]);
            seriesData.data.push([delayTime + peakDuration, tg.threadNumber]);
            delayTime += peakDuration;
          }
          seriesData.data.push([duration, tg.minThreadNumber])
          handler.options.series.push(seriesData);

        } else if (tg.model === 'dip') {
    ...
    validConfig() {
      ...
      for (let i = 0; i < this.threadGroups.length; i++) {
        ...
        if ((!tg.threadNumber || !tg.duration
          || !tg.rampUpTime || !tg.step || !tg.iterateNum) && !tg.model) {
          this.$warning(this.$t('load_test.pressure_config_params_is_empty'));
          this.$emit('changeActive', '1');
          return false;
        }

        if (tg.rpsLimitEnable && !tg.rpsLimit && !tg.model) {
          this.$warning(this.$t('load_test.pressure_config_params_is_empty'));
          this.$emit('changeActive', '1');
          return false;
        }
    ...
    convertProperty() {
      ...
      /// todo: 新增加的 MODEL SHOCK_COUNT WAVE_COUNT MIN_THREAD_NUMBER 的属性是jmeter UltimateThreadGroup plugin的属性
      let result = [];
      ...
      for (let i = 0; i < this.threadGroups.length; i++) {
        result.push([
          ...
          {key: MODEL, value: this.threadGroups[i].model},
          {key: SHOCK_COUNT, value: this.threadGroups[i].shockCount},
          {key: WAVE_COUNT, value: this.threadGroups[i].waveCount},
          {key: MIN_THREAD_NUMBER, value: this.threadGroups[i].minThreadNumber},
          {key: PEAK_DURATION, value: this.threadGroups[i].peakDuration},
          {key: RESPONSE_TIME, value: this.threadGroups[i].responseTime},
          {key: ERROR_RATE, value: 100 - this.threadGroups[i].errorRate}
        ]);
      }
