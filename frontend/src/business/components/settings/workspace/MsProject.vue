  ...
  <div v-loading="result.loading">
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="id" :label="$t('commons.id')" min-width="150" show-overflow-tooltip/>
      ...
      <el-form :model="form" :rules="rules" ref="form" label-position="right" label-width="140px" size="small">
        ...
        <el-form-item label="项目类型" prop="projectType">
          <el-select v-model="form.projectType">
            <el-option label="jmeter" value="jmeter"></el-option>
            <el-option label="httprunner" value="httprunner"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="Git地址" prop="gitPath" v-if="form.projectType === 'httprunner'">
          <el-input v-model="form.gitPath" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="私有token" prop="privateToken" v-if="form.projectType === 'httprunner'">
          <el-input v-model="form.privateToken" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
import {
  ...
  listenGoBack,
  removeGoBackListener, saveLocalStorage
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        gitPath: [
          {required: true, message: this.$t('请输入Git地址'), trigger: 'blur'},
          {pattern: /^(http(s)?:\/\/)?([\w-]+\.)+[\w-]+(:\d{2,5})?(\/?\#?[\w-.\/?%&=]*)?$/, message: this.$t('请输入合法的gitPath路径'), trigger: 'blur'}
        ],
        privateToken: [
          {required: true, message: this.$t('请输入Git私钥'), trigger: 'blur'}
        ]
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.$get("project/get/" + row.id, response => {
        let data = response.data;
        this.form.projectType = data.projectType || 'jmeter';
        if (this.form.projectType === 'jmeter'){
          this.form.gitPath = "";
          this.form.privateToken = "";
        } else if (this.form.projectType === 'httprunner'){
          this.form.gitPath = data.gitPath;
          this.form.privateToken = data.privateToken;
        }
      })
    },
    submit(formName) {
