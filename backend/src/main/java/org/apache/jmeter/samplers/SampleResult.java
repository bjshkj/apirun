...
public class SampleResult implements Serializable, Cloneable, Searchable {
    ...
    public String getResponseDateAsStringAndDecodeUnicode() {
        String responseDataString = getResponseDataAsString();
        try {
            JSONObject objJson = JSONObject.parseObject(responseDataString, Feature.OrderedField);  //保证转换之后数据顺序不变
            return (String) handlerJsonData(objJson, "");
        }catch (Exception e){
            log.warn("Response body processing exception: "+e);
     ...
     * 递归处理响应体中json解析
     */
    public Object handlerJsonData(Object objJson, String tagKey) {
        if (objJson instanceof JSONArray) {//若为json数组
            ...
            if (len > 0) {
                for (Object o : objArray) {
                    if (o instanceof JSONArray) {
                        ...
                        handlerJsonData(oArray, tagKey);
                    } else {
                        handlerJsonData(o, tagKey);
                    }
                }
            }
        } else if (objJson instanceof JSONObject) {//若为json对象
            JSONObject jsonObject = (JSONObject) objJson;
            for (String s : jsonObject.keySet()) {
                Object value = jsonObject.get(s);
                if (value instanceof String) {
                    if (value.toString().indexOf('\"')!=-1){
                        ((JSONObject) objJson).put(s,value);
                    }else {
                        value=decodeUnicode((String) value);
                        ((JSONObject) objJson).put(s,value);
                    }
                    ...
                    ((JSONObject) objJson).put(s, value);
                    handlerJsonData(objArray, s);
                } else if (value instanceof JSONObject) {
                    ((JSONObject) objJson).put(s, value);
                    handlerJsonData(value, s);
                }
            }
        }
