...
import io.apirun.api.dto.automation.ApiTestReportVariable;
import io.apirun.api.dto.scenario.request.RequestType;
import io.apirun.api.service.*;
...
import io.apirun.track.service.TestPlanTestCaseService;
...
//        TestResult testResult = new TestResult();
    ...
    private RequestResult getRequestResult(SampleResult result) {
        ...
        /**GET URL不进行解码**/
        if (requestResult.getMethod().equals(HttpMethod.GET.name())) {
            requestResult.setUrl(result.getUrlAsString());
            requestResult.setBody(result.getSamplerData());
        }else {
            try {
                /**关于URLDecoder.decode()方法解码，会将“+”转为空格**/
                requestResult.setUrl(URLDecoder.decode(result.getUrlAsString(),"UTF-8"));
                requestResult.setBody(URLDecoder.decode(result.getSamplerData(),"UTF-8"));
...
//        responseResult.setBody(result.getResponseDataAsString());
        responseResult.setBody(result.getResponseDateAsStringAndDecodeUnicode());
