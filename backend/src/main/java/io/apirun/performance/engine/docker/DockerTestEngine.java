package io.apirun.performance.engine.docker;

import com.alibaba.fastjson.JSON;
import io.apirun.base.domain.LoadTestReportResult;
import io.apirun.base.domain.LoadTestWithBLOBs;
import io.apirun.base.domain.TestResource;
import io.apirun.base.mapper.LoadTestReportResultMapper;
import io.apirun.commons.constants.ReportKeys;
import io.apirun.commons.constants.ResourceStatusEnum;
import io.apirun.commons.exception.MSException;
import io.apirun.commons.utils.CommonBeanFactory;
import io.apirun.commons.utils.UrlTestUtils;
...
import io.apirun.performance.engine.AbstractEngine;
import io.apirun.performance.engine.request.StartTestRequest;
...
public class DockerTestEngine extends AbstractEngine {
    ...
    public void start() {
        ...
        Object[] resourceRatios = resourceList.stream()
                ...
                .toArray();
        //保存一个 completeCount 防止指定节点分配策略测试报告提前结束。
        LoadTestReportResult completeCount = new LoadTestReportResult();
        completeCount.setId(UUID.randomUUID().toString());
        completeCount.setReportId(this.getReportId());
        completeCount.setReportKey(ReportKeys.ReportCompleteCount.name());
        completeCount.setReportValue("" + resourceRatios.length); //初始化一个completeCount，这个值用在data-streaming中
